...
func _ready() -> void:
	...
	for map in maps_name:
		...
		if !data.is_empty():
			...
			var _uuid_size: int = buffer.get_u32();
			buffer.seek(8 + _name_size);
			var _uuid: String = buffer.get_string(_uuid_size);
			buffer.seek(8 + _name_size + _uuid_size);
			var _seed: int = buffer.get_64();
			buffer.seek(16 + _name_size + _uuid_size);
			var _size: int = buffer.get_u8();
			var _data: PackedByteArray = data.slice(17 + _name_size + _uuid_size);
			var map_item: MapItem = map_item_scene.instantiate();
			map_item.world_name = _name;
			map_item.world_uuid = _uuid;
